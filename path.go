...
func (routeParser *routeParser) analyseParameterPart(pattern string, customConstraints ...CustomConstraint) (int, *routeSegment) {
	...
	if strings.IndexByte(pattern, paramConstraintStart) != -1 && strings.IndexByte(pattern, paramConstraintEnd) != -1 {
	...
	if parameterEndPosition > 0 {
		parameterConstraintStart = findNextNonEscapedCharPosition(pattern[0:parameterEndPosition], paramConstraintStart)
...
// findNextNonEscapedCharsetPosition search the next char position and skip the escaped characters
func findNextNonEscapedCharPosition(search string, char byte) int {
	...
	for pos > 0 && search[pos-1] == escapeChar {
		...
		nextPossiblePos := strings.IndexByte(search[pos+1:], char)
		if nextPossiblePos == -1 {
			return -1
		}
		// the previous character is taken into consideration
